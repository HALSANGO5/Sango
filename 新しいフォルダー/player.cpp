...
HRESULT InitPlayer(void)
{
	...
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		...
		g_aPlayer[nCntPlayer].bIsHitByGimmick = false;
...
void SetPlayer(D3DXVECTOR3 pos, float fWidth, float fHeight, D3DXCOLOR col, int type)
{
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		if(!g_aPlayer[nCntPlayer].bUse)
		{
			...
			g_aPlayer[nCntPlayer].size = D3DXVECTOR3(fWidth, fHeight, 0.0f);

			g_aPlayer[nCntPlayer].fSpeedX = PLAYER_SPEED_X;
			g_aPlayer[nCntPlayer].fSpeedY = PLAYER_SPEED_Y;
			g_aPlayer[nCntPlayer].move = D3DXVECTOR3(PLAYER_SPEED_X, PLAYER_SPEED_Y, 0.0f);
	...
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		if(g_aPlayer[nCntPlayer].bUse)
		{
			...
			case PLAYER_001:
				...
				g_aPlayer[nCntPlayer].size = D3DXVECTOR3(PLAYER_SIZE_X, PLAYER_SIZE_Y, 0.0f);
...
bool IsHitBySlope(D3DXVECTOR3 nextPos, D3DXVECTOR2 *Intersection, bool isFall)
{
	...
	for(int nCntWall = 0; nCntWall < MAX_WALL; nCntWall++)
	{
		...
		{
			if(g_pWallDataPlayer[nCntWall].nType == WALL_TYPE_SLOPE_45)
			{
				...
				if(IsHitByBox(nextPos, g_pWallDataPlayer[nCntWall].pos, g_aPlayer[0].size, tmpSize))
				{
					...
					else
					{
						...
						//坂道の上にあったが、ブロックの上にあるようになる瞬間
						//高さ補正
							if(g_aPlayer[0].bIsOnSlope == true && g_aPlayer[0].bIsHit == true)
							{
								//プレイヤーの上辺が坂道ブロックの上辺より小さいとき
								if((g_aPlayer[0].pos.y - g_aPlayer[0].size.y /2) < g_pWallDataPlayer[nCntWall].pos.y - g_pWallDataPlayer[nCntWall].fHeight / 2)
								{
				...
				//坂道ブロックとあたっていないため、坂道の上でも、ブロックと当たってもない
				else
				{
					g_aPlayer[0].bIsOnSlope	= false;
					g_aPlayer[0].bIsHit = false;
