...
#define	WALL_SLOPE_45_SIZE_X	(200.0f)								//坂道ブロックの幅
#define	WALL_SLOPE_45_SIZE_Y	(200.0f)								//坂道ブロックの高さ
...
void SetVertexWall(	int nIdx,						// 今のテクスチャの配列番号			记录当前对应的图组polygon地址
					float scaleX,					// テクスチャのX軸のスケール		X轴的比例参数
					float scaleY);					// テクスチャのY軸のスケール		Y轴的比例参数

void SetTextureWall(int nIdx,						// 今のテクスチャの配列番号			记录当前对应的图组地址
...
const char *c_pTextureWall[WALL_TYPE_MAX]=
{
	...
	"data/TEXTURE/background/wall_100_100.png",			// 壁
	"data/TEXTURE/background/slope_45_100_100.png",		// 坂道45度
...
void UpdateWall(void)
{
	...
	for(int nCntWall = 0; nCntWall < MAX_WALL; nCntWall++)
	{
		if(g_aWall[nCntWall].bUse)
		{
			...
			if(g_aWall[nCntWall].nType == WALL_TYPE_SLOPE_45)
			{
				g_aWall[nCntWall].fMaxX = g_aWall[nCntWall].pos.x + WALL_SLOPE_45_SIZE_X / 2;
				g_aWall[nCntWall].fMinX = g_aWall[nCntWall].pos.x - WALL_SLOPE_45_SIZE_X / 2;
				g_aWall[nCntWall].fMaxY = g_aWall[nCntWall].pos.y + WALL_SLOPE_45_SIZE_Y / 2;
				g_aWall[nCntWall].fMinY = g_aWall[nCntWall].pos.y - WALL_SLOPE_45_SIZE_Y / 2;
	...
	for(int nCntWall = 0; nCntWall < MAX_WALL; nCntWall++)
	{
		if(!g_aWall[nCntWall].bUse)
		{
			...
			case WALL_TYPE_001:
				...
				strcpy(g_aWall[nCntWall].aRemarks, "WALL_TYPE_001");
				break;
			...
			case WALL_TYPE_002:
				...
				g_aWall[nCntWall].fMinY = g_aWall[nCntWall].pos.y - g_aWall[nCntWall].fHeight / 2;
				g_aWall[nCntWall].nType = type;
				strcpy(g_aWall[nCntWall].aRemarks, "WALL_TYPE_002");
				break;
			
			case WALL_TYPE_SLOPE_45:
				g_aWall[nCntWall].fWidth = WALL_SLOPE_45_SIZE_X;
				g_aWall[nCntWall].fHeight = WALL_SLOPE_45_SIZE_Y;	
				g_aWall[nCntWall].fMaxX = g_aWall[nCntWall].pos.x + g_aWall[nCntWall].fWidth / 2;
				g_aWall[nCntWall].fMinX = g_aWall[nCntWall].pos.x - g_aWall[nCntWall].fWidth / 2;
				g_aWall[nCntWall].fMaxY = g_aWall[nCntWall].pos.y + g_aWall[nCntWall].fHeight / 2;
				g_aWall[nCntWall].fMinY = g_aWall[nCntWall].pos.y - g_aWall[nCntWall].fHeight / 2;
				...
				g_aWall[nCntWall].fSteep = (endPoint.y - startPoint.y) / (endPoint.x - startPoint.x);//傾きの設定
				g_aWall[nCntWall].nType = type;
				strcpy(g_aWall[nCntWall].aRemarks, "WALL_TYPE_SLOPE_45");
				break;
...
void WallCreateMode(void)
{
	...
	if(g_bIsWallCreating == true)
	{
		...
		if(IsMouseLeftPressed())
		{
			...
			if(bTemp == true)
			{
				...
				else if(g_aWall[nTemp].nType == WALL_TYPE_SLOPE_45)
				{
					fX = *GetMouX() - (int)*GetMouX() % ((int)g_aWall[nTemp].fWidth / 10);
					fY = *GetMouY() - (int)*GetMouY() % ((int)g_aWall[nTemp].fWidth / 10);
				}
	...
	if(IsMouseRightTriggered())
	{
		...
		if(g_nCreatingNum != -1)
		{
			...
			switch (nType)
			{
				...
				case WALL_TYPE_SLOPE_45:
					g_nCreatingNum = SetWall(D3DXVECTOR3(*GetMouX(), *GetMouY(), 0.0f),
						D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), WALL_TYPE_SLOPE_45, "WALL_TYPE_SLOPE_45");
					break;
				case WALL_TYPE_GOAL:
					g_nCreatingNum = SetWall(D3DXVECTOR3(*GetMouX(), *GetMouY(), 0.0f),
	...
	if(g_bIsWallCreating == false)
	{
		...
		else if(g_aWall[g_nCreatingNum].nType == WALL_TYPE_SLOPE_45)
		{
			fX = *GetMouX() - (int)*GetMouX() % ((int)g_aWall[g_nCreatingNum].fWidth / 4);
			fY = *GetMouY() - (int)*GetMouY() % ((int)g_aWall[g_nCreatingNum].fWidth / 4);
		}
	...
	switch(g_aWall[g_nCreatingNum].nType)
	{
		...
		case WALL_TYPE_002:
			...
			break;
		case WALL_TYPE_SLOPE_45:
			PrintDebugProc("【OBJECT NAME】: WALL_TYPE_SLOPE_45\n");
			break;
		case WALL_TYPE_GOAL:
